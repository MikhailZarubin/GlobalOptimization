...
class IndexAlgorithmForMultidimensionalFunc
{
	//original function
	Function function;
	//function border
	std::vector<long double> leftBorder, rightBorder;
	//constraint functions and coeff
	std::vector<Function> constraintFunctions;
	//a lot of proven points (for the first step of the algorithm)
	std::set<std::pair<long double, pointValues>, bool(*) (const std::pair<long double, pointValues>&, const std::pair<long double, pointValues>&)> pointContainer;
	...
	std::vector<long double> parseArguments(long double basicPoint);

	//cleaning from old values
	void clean();

public:
	IndexAlgorithmForMultidimensionalFunc(const std::string& expressionAndValue, const std::vector<Function>& constraintsAndCoeff,
		std::vector<long double> leftBorder, std::vector<long double> rightBorder, int dimension = 2, const std::string& variableSet = "xy", long double accuracy = 0.001, long double coeff_ = 2);
	std::pair<std::pair<long double, long double>, int> startIndexAlgorithm();
	std::vector<double> getPoints() const;
};
